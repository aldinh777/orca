export const ERROR_CODES: any = {
    ...
    MODEL_CLONE_JUTSU: (oldname: string, newname: string) =>
        ...
        `already exists, you can't do that or else you have 2 models with the same name.` +
    ...
    INSERT_INVALID_COLUMN: (colname: string, model: string, o: any) =>
        `imvalid column '${colname}' when insert into model '${model}'\n` +
    ...
    REFS_ADD_MODEL_MISMATCH: (colname: string) => `model row mismatch when trying to add ref '${colname}'`,
